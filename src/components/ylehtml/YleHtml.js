...
export default class YleHtml extends Component {
 ...
 //   if (document.getElementById("programtable"))
  ...
  getJsonProgram(nodeProgram) {
    ...
    if (nodesMeta)
      for (var i = 0; i < nodesMeta.length; i++) {
        ...
        if (nodeMeta) {
          ...
          if (
            nodeMeta.getAttribute("itemprop") == "accessibilityFeature" &&
    ...
    jsonProgram.movie =
      itemTypeStr && (itemTypeStr.includes("Movie") ? true : false);

    jsonProgram.status = nodeProgram.className;
    let nodeHeader = nodeProgram.querySelector(
      'div[class="schedule-card__header"]'
    );
    if (nodeHeader) {
      let nodeSpan = nodeProgram.querySelector("span");
      if (nodeSpan) {
        let nodePubl = nodeSpan.querySelector(
          'span[class="schedule-card__publication"]'
        );
        if (nodePubl) {
          jsonProgram.startTime = nodePubl.textContent
            .toString()
            .replaceAll("\n", "")
            .trim();
          // <time itemprop="startDate"
          let nodeStartDate = nodePubl.querySelector(
            'time[itemprop="startDate"]'
          );
          if (nodeStartDate)
        ...
        let nodeTitle = nodeSpan.querySelector(
          'span[class="schedule-card__title"]'
        );
        if (nodeTitle) {
          jsonProgram.name = nodeTitle.textContent
            .toString()
            .replaceAll("\n", "")
            .trim();
    ...
    let nodeFooter = nodeProgram.querySelector(
    ...
    if (nodeFooter) {
        ...
        for (var i = 0; i < nodesLink.length; i++) {
          ...
          if (nodeLink) {
            ...
            if (nodeLink.href) {
              ...
              jsonLink.text = nodeLink.textContent
                .toString()
                .replaceAll("\n", "")
                .trim();
      ...
      let nodeDuration = nodeFooter.querySelector(
        'span[class="schedule-card__duration"]'
      );
      if (nodeDuration)
        jsonProgram.durationtext = nodeDuration.textContent
          .toString()
          .replaceAll("\n", "")
          .trim();
      let nodeDescription = nodeFooter.querySelector(
        'span[itemprop="description"]'
      );
      if (nodeDescription) {
        jsonProgram.description = nodeDescription.textContent
          .toString()
          .replaceAll("\n", "")
          .trim();
      }
      ...
      if (nodeName)
        jsonProgram.captiontext = nodeName.textContent
          .toString()
          .replaceAll("\n", "")
          .trim();
    }
    ...
    if (nodeH2) {
      ...
      if (nodeA) {
        ...
        if (nodeDiv) {
          ...
          if (xmlStr1) jsonChannel.name = nodeA.textContent;
          else {
            if (Config.bDebug) console.log("kissa");
          }
        }
      } else {
        if (Config.bDebug) console.log("kissa2");
      }
    }

    if (!jsonChannel.name) {
      let nomoreprogs = xmlchannels.querySelector(
        '*[class="schedule-card__no-more-programs-today"]'
      );
      if (nomoreprogs) {
  ...
  getJsonDataFromYleXml = (data) => {
    ...
    let xmlchannels = nodeLu.querySelectorAll(
  ...
  fetchProgSchedules = (progsource, selecteddate) => {
    ...
    if (Config.bDebug) {
      ...
      console.log(
        fetch_url + this.getTypeParamAndValue(progsource, selecteddate)
      );
    }
...
*/
  ...
  getDayGrid = () => {
    return (
      <DayServices
        selecteddate={this.store.getState().selecteddate}
  ...
  onClickSetDateStringYle = (event) => {
    ...
    } else if (this.checkshowdcurrentprogramsRef.current.disabled)
      ...
      let tmp_channels = state.services.map((s2, k) => {
        return (
          <YleChannel
            ...
            yleapiparams={this.props.yleapiparams}
  ...
  radioProgSourceChanged = (event) => {
    ...
    if (this.checkBoxMovieRef.current) {
      ...
      } else this.checkBoxMovieRef.current.disabled = false;
    }
  ...
  showOnlyMovies = (event) => {
    event.preventDefault();
    let value = event.target.checked;
    if (Config.bDebug) console.log("showOnlyMovies");
    let bValue = value;
    if (Config.bDebug) {
      console.log("bValue");
      console.log(bValue);
  ...
  getTableHeadersAndTableRowsAfterChannels = () => {
    ...
    if (all_channels == null) {
      ...
      if (!selchanenl) {
        console.log("stop");
      } else if (!selchanenl.name) {
        console.log("stop2");
      }
      let selectedTitle =
        selchanenl && selchanenl.name ? selchanenl.name : null;
  ...
  calculateMaxChannelSetIndex = () => {
    ...
    if (this.state.schedules == null) return 0;
    ...
    if (this.state.showChannelsAtSameTime >= this.state.schedules.length)
      return 0;
    const channelSets =
      this.state.schedules.length / this.state.showChannelsAtSameTime;
    if (Config.bDebug) {
      console.log("channelSets");
      console.log(channelSets);
    }
        ...
        if (searchText == null || searchText.toString().trim().length == 0) {
          ...
          Array.from(search_channels).forEach((cha) => {
            ...
            /*
						...
						*/
            index = chcoopy.name
              ? chcoopy.name.toUpperCase().indexOf(uppersearchText)
              : -1;
            if (index != -1) {
            ...
            Array.from(chcoopy.channelprograms).forEach((pr) => {
              ...
              index = prcopy.name
                ? prcopy.name.toString().toUpperCase().indexOf(uppersearchText)
                : -1;
              if (index != -1) {
  ...
  searchTextFromChannelsClicked = (event) => {
    ...
    if (!value) currcntl.checked = true;
...
//              if (buttonPrev) buttonPrev.click();
  ...
  render(props, state) {
    ...
    else if (!state.bCheckShowChannelsAtSameTime) {
      selectedchannelindex = 0;
    ...
    const storestate = this.store.getState();
    if (
      storestate.schedules != undefined &&
      storestate.schedules !== null &&
      ...
      selectchannels = all_channels.map((child, i) => {
        return (
          <option value={child.name} selected={i === selectedchannelindex}>
            {child.name}
          </option>
        );
      });
          ...
          >
            ...
            <div role="radiogroup" aria-labelledby="idyleprogramdatasource">
              ...
              <input
                ...
                role="radio"
                style={inputw}
                checked={state.progsource === "rradio"}
                aria-checked={state.progsource === "rradio"}
                value="rradio"
                onChange={this.radioProgSourceChanged}
              >
              ...
              <div class={style.cardHeader}>
                ...
                <div class={style.cardHeader} >
                  <a
                    ...
                    lang="fi"
                    aria-labelledby={this.getPlus1Day(0)}
                  ...
                  <a
                    ...
                    lang="fi"
                    aria-labelledby={this.getPlus1Day(1)}
                    onClick={this.onClickSetDateStringYle}
                  >
                    {this.getPlus1Day(1)}
                  </a>
                  ...
                  <a
                    ...
                    lang="fi"
                    aria-labelledby={this.getPlus1Day(2)}
                    onClick={this.onClickSetDateStringYle}
                  >
                    {this.getPlus1Day(2)}
                  </a>
                  ...
                  <a
                    ...
                    lang="fi"
                    aria-labelledby={this.getPlus1Day(3)}
                    onClick={this.onClickSetDateStringYle}
                  >
                    {this.getPlus1Day(3)}
                  </a>
                  ...
                  <a
                    ...
                    lang="fi"
                    aria-labelledby={this.getPlus1Day(4)}
                    onClick={this.onClickSetDateStringYle}
                  >
                    {this.getPlus1Day(4)}
                  </a>
                  ...
                  <a
                    ...
                    lang="fi"
                    aria-labelledby={this.getPlus1Day(5)}
                    onClick={this.onClickSetDateStringYle}
                  >
                    {this.getPlus1Day(5)}
                  </a>
                  ...
                  <a
                    ...
                    lang="fi"
                    aria-labelledby={this.getPlus1Day(6)}
                    onClick={this.onClickSetDateStringYle}
                  >
                    {this.getPlus1Day(6)}
                  </a>
                  ...
                  <a
                    ...
                    lang="fi"
                    aria-labelledby={this.getPlus1Day(7)}
                    onClick={this.onClickSetDateStringYle}
                  >
                    {this.getPlus1Day(7)}
                  </a>
                  ...
                  <a
                    ...
                    lang="fi"
                    aria-labelledby={this.getPlus1Day(8)}
                    onClick={this.onClickSetDateStringYle}
                  >
                    {this.getPlus1Day(8)}
                  </a>
                  ...
                  <a
                    ...
                    lang="fi"
                    aria-labelledby={this.getPlus1Day(9)}
                    onClick={this.onClickSetDateStringYle}
                  >
                    {this.getPlus1Day(9)}
                  </a>
                  ...
                  <a
                    ...
                    lang="fi"
                    aria-labelledby={this.getPlus1Day(10)}
                    onClick={this.onClickSetDateStringYle}
                  >
                    {this.getPlus1Day(10)}
                  </a>
                  ...
                  <a
                    ...
                    lang="fi"
                    aria-labelledby={this.getPlus1Day(11)}
                    onClick={this.onClickSetDateStringYle}
                  >
                    {this.getPlus1Day(11)}
                  </a>
                  ...
                  <a
                    ...
                    lang="fi"
                    aria-labelledby={this.getPlus1Day(12)}
                    onClick={this.onClickSetDateStringYle}
                  >
                    {this.getPlus1Day(12)}
                  </a>
                  ...
                  <a
                    ...
                    lang="fi"
                    aria-labelledby={this.getPlus1Day(13)}
                    onClick={this.onClickSetDateStringYle}
                  >
                    {this.getPlus1Day(13)}
                  </a>
                  ...
                  <a
                    ...
                    lang="fi"
                    aria-labelledby={this.getPlus1Day(14)}
                    onClick={this.onClickSetDateStringYle}
                  >
                    {this.getPlus1Day(14)}
                  </a>
                ...
                <div class={style.cardHeader}>
                  ...
                  <a
                    href="."
                    id={"dayname_" + this.getPlus1DayId(-15)}
                    lang="fi"
                    aria-labelledby={this.getPlus1Day(-15)}
                    onClick={this.onClickSetDateStringYle}
                  >
                    {this.getPlus1Day(-15)}
                  </a>
                ...
                <div>
                  ...
                  <span>
                    ...
                    <input
                      ...
                      id="idsearchvalue"
                      disabled={
                        this.store.getState().schedules == null ||
                        state.bUnderFetch
                      }
