...
//				channels: this.state.programtypeitems });
	...
	render(props, state) {
		...
		const buttonFocusStyle = (props.themevalue !== undefined && props.themevalue !== '' ? 
		...
		{  // selected={child.title.fi===state.selectedcategory.title.fi} 
           ...
           //	>{child.title}</Select.Item>
			selectchannels = state.channels.map((child, i) => {
				return <option selected={i===selectedchannelindex}
				value={child.title} id={i} >{child.title}</option>
			});
		...
		*/
		...
		return (
			<Fragment>
			<div id="amppari_main_div" style={" min-height: 100%; margin: 0px; height: 100%: " +divDialogStyle}>
			<div class={style.cardHeader +divDialogStyle} onKeyUp={this.altPlusKeyUpProgramHeader}>
				...
				<div>
					...
					<table id="programtable" style={"width:100%; " +tableBorderStyle} style={this.state.themevalue === '' ? '' : this.sectionStyle}
							ref={this.tablCntl}>
							<tbody>
							<tr>{tableheaders}</tr>
							<tr>{tabletds}</tr> 		
