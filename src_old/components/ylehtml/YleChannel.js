...
export default function YleChannel(props) {
  ...
  const getPrograms = (programs) => {
    if (programs == undefined || programs === null) return null;
    ...
    let progs = programs.map((s, k) => {
      ...
      if (
        props.data &&
        s.status != "schedule-list__no-data"
        ...
        return (
          <YleProgram
            ...
            showSearch={
              props.showSearch == undefined ? false : props.showSearch
            }
    ...
    if (props.bShowOnlyMovies && progs) {
      ...
      Array.from(progs).forEach((item) => {
        ...
        if (item.props.data.movie) {
          if (Config.bDebug) console.log("movie");
          progs2.push(item);
        }
      });
      progs = progs2;
    ...
    if (props.bshowdcurrentprograms && progs) {
      ...
      Array.from(progs).forEach((item) => {
        ...
        if (!oldProgram(item.props.data, props.currenttime)) {
          if (Config.bDebug) console.log("!oldProgram");
          progs1.push(item);
        }
      });
      progs = progs1;
  ...
  const onfocusoutfunc = (e) => {
    if (Config.bDebug) console.log("onfocusoutfunc");
  };

  const onfocusfunc = (e) => {
    if (Config.bDebug) console.log("onfocusfunc");
  };
    ...
    <div>
      ...
      >
        {props.showSearch && props.data.titleindex != undefined
          ? props.getPOfIndex(
              props.data.titleindex,
              props.data.name,
              this.props.themevalue
            )
          : props.data.name}
      </h3>
      {props.data && props.data.nomoreprogramstoday && props.data.desciption ? (
        <div>{props.data.desciption}</div>
      ) : null}
